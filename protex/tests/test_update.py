...
def test_ff_parameters(caplog):
    caplog.set_level(logging.DEBUG)

    simulation = generate_im1h_oac_system()
    for force in simulation.system.getForces():
        if type(force).__name__ == "NonbondedForce":
            for id in range(force.getNumParticles()):
                params = force.getParticleParameters(id)
                print(params)
                quit()
    ...
    PDBFile.writeFile(ionic_liquid.topology, state.getPositions())
    # ionic_liquid.saveState("state_before_update.xml")
    # candidate_pairs = state_update.update(2)
    # ionic_liquid.saveState("state_after_update.xml")
    # print(candidate_pairs)
