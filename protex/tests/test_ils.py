...
def test_run_simulation(tmp_path):
    ...
    simulation = generate_im1h_buta_system(use_plugin=False, restart_file=f"{protex.__path__[0]}/forcefield/buth.rst")
...
def test_create_IonicLiquidTemplate():
    allowed_updates = {}
    allowed_updates[frozenset(["IM1H", "BUTA"])] = {"r_min": 0.135, "r_max": 0.145, "prob": 0.999}  # r_max in nanometer, prob zwischen 0 und 1
    allowed_updates[frozenset(["IM1", "BUTH"])] = {"r_min": 0.139, "r_max": 0.151, "prob": 0.070} 
    allowed_updates[frozenset(["IM1H", "IM1"])] = {"r_min": 0.159, "r_max": 0.185, "prob": 0.201} 
    allowed_updates[frozenset(["BUTH", "BUTA"])] = {"r_min": 0.140, "r_max": 0.152, "prob": 0.754} 


    templates = ProtexTemplates(
        [BUTH_BUTA, IM1H_IM1], (allowed_updates)
    )

    r = templates.get_residue_name_for_coupled_state("BUTA")
    assert r == "BUTH"
    r = templates.get_residue_name_for_coupled_state("BUTH")
    assert r == "BUTA"
    ...
    assert templates.states["BUTA"]["atom_name"] == "O2"
    assert templates.states["BUTH"]["atom_name"] == "H8"

    assert sorted(templates.names) == sorted(
        ["BUTA", "BUTH", "IM1H", "IM1"]
    )
    print(templates.allowed_updates)
    assert templates.overall_max_distance == 0.185

    # neutral_prob = templates.get_update_value_for(frozenset(["IM1", "BUTH"]), "prob")
    # assert neutral_prob == 1
    # ionic_prob = templates.get_update_value_for(frozenset(["IM1H", "BUTA"]), "prob")
    # assert ionic_prob == 1
    # ionic_prob = templates.get_update_value_for(frozenset(["BUTH", "BUTA"]), "prob")
    # assert ionic_prob == 1


def test_create_IonicLiquid():
    psf_for_parameters = f"{protex.__path__[0]}/forcefield/buth.psf"
    crd_for_parameters = f"{protex.__path__[0]}/forcefield/buth.crd"
    ...
    # )
    PARA_FILES = [
                "toppar_drude_master_protein_2019g_lj04.str",
                "buta.str",
                "buth.str",
                "im1h.str",
                "im1.str",
            ]
    para_files = [f"/site/raid7/marta/students/tanja/pt/toppar/{para_file}" for para_file in PARA_FILES]

    simulation = generate_im1h_buta_system(para_files=para_files,
        use_plugin=False
    )  # psf_file=psf_file)
   
    simulation_for_parameters = generate_im1h_buta_system(
...
def test_save_load_system(tmp_path):
    # psf_for_parameters = f"{protex.__path__[0]}/forcefield/hpts_single/hpts_single.psf"
    psf_for_parameters = f"{protex.__path__[0]}/forcefield/psf_for_parameters.psf"
    psf_file = f"{protex.__path__[0]}/forcefield/hpts.psf"
    crd_for_parameters = f"{protex.__path__[0]}/forcefield/crd_for_parameters.crd"
    ...
    while i < 5:  # eventuell change to 20
        ...
        simulation = generate_hpts_meoh_system(psf_file=psf_file,use_plugin=False)
        simulation_for_parameters = generate_hpts_meoh_system(
            psf_file=psf_for_parameters, crd_file=crd_for_parameters,use_plugin=False
        )
        #simulation = generate_single_hpts_meoh_system(psf_file=f"{tmp_path}/old_psf.psf")
        if i == 0:
            ionic_liquid = ProtexSystem(simulation, templates, simulation_for_parameters)
           ...
           state_update = StateUpdate.load(f"{tmp_path}/state_update_{i-1}.pkl", update)
           ionic_liquid = ProtexSystem.load(f"{tmp_path}/system_{i-1}.pkl", simulation, simulation_for_parameters)
           ionic_liquid.load_updates(f"{tmp_path}/updates_{i-1}.txt")
           ionic_liquid.loadState(f"{tmp_path}/state_{i-1}.rst") 
        ...
        #     f"{tmp_path}/old_psf.psf", f"{tmp_path}/test.psf", psf_for_parameters
        # )
        ionic_liquid.saveState(f"{tmp_path}/state_{i}.rst")
        ionic_liquid.saveCheckpoint(f"{tmp_path}/checkpoint_{i}.rst")
        state_update.dump(f"{tmp_path}/state_update_{i}.pkl")
        ionic_liquid.dump(f"{tmp_path}/system_{i}.pkl")
        ionic_liquid.save_updates(f"{tmp_path}/updates_{i}.txt")
