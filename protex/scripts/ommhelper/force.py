...
def slab_correction(system):
    ...
    * The box size should not change during the simulation.
    ...
    _conv = (1 / (4 * CONST.PI * _eps0) * qe**2 / nm / unit.item).value_in_unit(
        kJ_mol
    )
...
def spring_self(system, positions, particles, strength):
    ...
    if system.getNumParticles() != len(positions):
        raise Exception(
            "Length of positions does not equal to number of particles in system"
        )
    ...
    force = mm.CustomExternalForce(
        f"{kx}*periodicdistance(x,0,0,x0,0,0)^2+"
...
def CLPolCoulTT(system, donors, b=45.0):
    ...
    """
    nbforce: mm.NonbondedForce = next(
        f for f in system.getForces() if type(f) == mm.NonbondedForce
    )
    dforce: mm.DrudeForce = next(
        f for f in system.getForces() if type(f) == mm.DrudeForce
    )
    ...
    for i in range(dforce.getNumParticles()):
        (
            ...
            aniso12,
            aniso34,
        ) = dforce.getParticleParameters(i)
        drude_pairs[parent] = drude
        dipole_set.add(parent)
        dipole_set.add(drude)

    ttforce = mm.CustomNonbondedForce(
        "-%.6f*q1*q2/r*beta*gamma;"
        "beta=exp(-br);"
...
def restrain_particle_number(
    system, particles, direction, bound, sigma, target, k, weights=None
):
    """
