...
class _OpenMMReadInputs():
    ...
    def __init__(self):
        ...
        self.dt               = 0.0005                                     # Time-step (ps)
        self.pre_nstep        = 0                                         # Number of steps to run
        self.pre_dt           = 0.0001                                     # Time-step (ps)

        self.nstout           = 100                                       # Writing output frequency (steps)
        self.nstdcd           = 0                                         # Wrtiing coordinates trajectory frequency (steps)

        self.boxl             = 48.0                                      # Boxlength (A)
        ...
        self.r_on             = 10                                       # Switch-on distance (A)
        self.r_off            = 12                                       # Switch-off distance (A)

        self.temp             = 300.0                                     # Temperature (K)
        self.fric_coeff       = 5                                         # Friction coefficient for Langevin dynamics
        self.coll_freq        = 10                                        #collision frequency for DrudeNoseHoover Integrator

        self.drude_temp       = 1.0                                       # Drude Temperature (K)
        self.drude_fric_coeff = 20                                        # Drude Friction coefficient for Langevin dynamics
        self.drude_coll_freq  = 200                                       # Drude collision frequency for DrudeNoseHoover Integrator
    ...
    def read(self, inputFile):
        for line in open(inputFile, 'r'):
            ...
            if len(line) > 0:
                ...
                if input_value:
                    if input_param == 'ENSEMBLE':                       self.ensemble         = str(input_value)
                    ...
                    if input_param == 'PRE_NSTEP':                      self.pre_nstep        = int(input_value)
                    if input_param == 'PRE_DT':                         self.pre_dt           = float(input_value)
                    ...
                    if input_param == 'BOXL':                           self.boxl             = float(input_value)
                    ...
                    if input_param == 'COLL_FREQ':                      self.coll_freq        = float(input_value)
                    ...
                    if input_param == 'DRUDE_COLL_FREQ':                self.drude_coll_freq = float(input_value)
