...
#python msd_neutral.py [--insidebash]
...
if args.insidebash:
    ...
    base="/site/raid2/florian/pt/tests/dnh_10_200/"
    
    psf=base+"im1h_oac_200_im1_hoac_300.psf"
    dcd_base = "traj/im1h_oac_200_im1_hoac_300_"
    dcd = ["%s%d_write5.dcd" % (base+dcd_base,i) for i in range(firstfile,lastfile+1)]
    print("PSF file:", psf)
...
# molecular species
#################################################################################################################################
sel_cat = u.select_atoms("resname IM1")
sel_an  = u.select_atoms("resname HOAC")
...
filename1 = "im1_msd_{}-{}_skip{}.dat".format(firstfile, lastfile, skip)
filename2 = "hoac_msd_{}-{}_skip{}.dat".format(firstfile, lastfile, skip)
f1=open(filename1,'w')
f2=open(filename2,'w')
