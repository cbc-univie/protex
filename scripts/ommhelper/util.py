...
def minimize(sim, tolerance, gro_out=None):
    ...
    if gro_out is not None:
        GroFile.writeFile(
            sim.topology, state.getPositions(), state.getPeriodicBoxVectors(), gro_out
        )
    ...
    if pcoupl == "iso":
        ...
        system.addForce(
            mm.MonteCarloAnisotropicBarostat(
                [P * bar] * 3, T * kelvin, True, True, True, nsteps
            )
        )
    elif pcoupl == "xy":
        ...
        system.addForce(
            mm.MonteCarloAnisotropicBarostat(
                [P * bar] * 3, T * kelvin, True, True, False, nsteps
            )
        )
    elif pcoupl == "z":
        ...
        system.addForce(
            mm.MonteCarloAnisotropicBarostat(
                [P * bar] * 3, T * kelvin, False, False, True, nsteps
            )
        )
    else:
