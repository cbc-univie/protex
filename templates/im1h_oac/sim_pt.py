...
to_adapt = [
    ("IM1H", 150, frozenset(["IM1H", "OAC"])),
    ("IM1", 350, frozenset(["IM1", "HOAC"])),
]
...
ionic_liquid.simulation.reporters.append(
    DCDReporter(f"traj/nvt_{file_nr}.dcd", int(dcd_save_freq))
)
ionic_liquid.simulation.reporters.append(
    StateDataReporter(
        sys.stdout,
...
ionic_liquid.simulation.reporters.append(
    DrudeTemperatureReporter(f"out/drude_temp_{file_nr}.out", state_save_freq)
)
...
if update_steps % 2 != 0:
    warnings.warn(
        "Probably working, not entirely tested. To be on the safe side use an even number for 'update_steps'",
        UserWarning,
    )
...
)  # int division not working, dt too small
if ((1 / dt) * time_between_updates) % (
    1 / dt
) != 0:  # BUG not working if time_between_updates <1
...
real_steps_between_updates = (
    steps_between_updates - update_steps
)  # remove the steps taken during the updates process
...
if file_nr == 1:
    actual_transfer_cycles = (
        transfer_cycles - 1
    )  # -1 da die ersten steps außerhalb der loop stattfinden
    assert (
        total_steps
        == init_steps_between_updates
        + (real_steps_between_updates + update_steps) * (actual_transfer_cycles)
        + final_steps_between_updates
    )
    simulation.context.setTime(0.0)
else:
    actual_transfer_cycles = transfer_cycles
    assert total_steps == (real_steps_between_updates + update_steps) * (
        actual_transfer_cycles
    )
...
infos = {
    "dcd_save_freq": dcd_save_freq,
    "sim_time(ps)": sim_time,
    "update_steps": update_steps,
    "steps_between_updates": steps_between_updates,
    "dt(ps)": dt,
}
charge_reporter = ChargeReporter(
    f"out/charge_changes_{file_nr}.out", dcd_save_freq, ionic_liquid, header_data=infos
)
